(module renderer-logic
  ...
  (define set-pixel
	...
	; bgr
	; bgr
	(u8vector-set! fb (+ (x-y-to-index x y) 2) (quotient (+ (* r a) (* dst-red (- 255 a)))  255))
	(u8vector-set! fb (+ (x-y-to-index x y) 1) (quotient (+ (* g a) (* dst-green (- 255 a)))  255))
	(u8vector-set! fb (x-y-to-index x y) (quotient (+ (* b a) (* dst-blue (- 255 a)))  255))
	)
      ))
  ...
  (define parse-image-header
	...
	(list
	 (subu8vector image-with-header 17 (u8vector-length image-with-header))
	 width-acc
	 heigth-acc
	 )
	)
